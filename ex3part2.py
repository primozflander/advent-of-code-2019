...
def getCoordinates(input):
    ...
    for string in input:
        ...
        if string[0] == 'R':
            x.append(list(range(cx, cx + diff, 1)))
            y.append([cy] * diff)
            cx += diff
            ...
            x.append([cx] * diff)
            y.append(list(range(cy, cy + diff, 1)))
            cy += diff
...
#     if c1[i] in c2:
#         results.append(c1[i])
results = set(c1).intersection(set(c2))
...
for j in results:
    for i,element in enumerate(c1):
        if element == j:
            steps_input1.append((i, element))

#steps_input1_sorted = sorted(steps_input1, key=lambda x:x[0])
print(steps_input1)

steps_input2 = list()

for j in results:
    for i,element in enumerate(c2):
        if element == j:
            steps_input2.append((i, element))

#steps_input2_sorted = sorted(steps_input2, key=lambda x:x[0])
print(steps_input2)

sum_step = list()

for i in range(len(steps_input1)):
    sum_step.append(steps_input1[i][0] + steps_input2[i][0])

print('sum step:',sorted(sum_step))
# sorted_results = sorted(results)
# print(sorted_results)
#
# abs_results = list()
# for x,y in results:
#     abs_results.append(abs(x) + abs(y))
#
# abs_results = [abs(x)+abs(y) for x,y in results]

t = [c1.index(result) + c2.index(result) for result in results]
print("t", sorted(t))
#print(sorted(abs_results))
# sorted_reults = sorted(results)
# print(sorted_results)
# print(abs(sorted_results[0][0])+abs(sorted_results[0][1]))
